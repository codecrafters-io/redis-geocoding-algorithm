...
int main() {
    ...
    for (int i = 0; i < num_tests; i++) {
        coordinates_t result = decode(test_cases[i].score);
        
        // Check if decoded coordinates are close to original (within 10e-6 precision)
        double lat_diff = fabs(result.latitude - test_cases[i].expected_latitude);
        double lon_diff = fabs(result.longitude - test_cases[i].expected_longitude);
        
        int success = (lat_diff < 1e-6 && lon_diff < 1e-6);
        printf("%s: (lat=%.15f, lon=%.15f) (%s)\n", 
               test_cases[i].name, result.latitude, result.longitude, 
               success ? "✅" : "❌");
        
        if (!success) {
            printf("  Expected: lat=%.15f, lon=%.15f\n", 
                   test_cases[i].expected_latitude, test_cases[i].expected_longitude);
